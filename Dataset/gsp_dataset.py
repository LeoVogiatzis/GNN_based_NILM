...
class NilmDataset(Dataset):
    ...
    def process(self):
        ...
        for raw_path in self.raw_paths:
            ...
            # exit(np.unique(labels).reshape(-1))
            ...
            # exit(np.unique(data.y).reshape(-1))
    ...
    def _get_adjacency_info(self, data_vec):
        ...
        # N = np.asarray(data_aggr).shape
        # freq_data = fft(data_aggr)
        # freq_data = freq_data / np.linalg.norm(freq_data)
        # delta_p = [np.round(np.sqrt((np.real(freq_data[i + 1]) - np.real(freq_data[i])) ** 2 + (
        #         np.imag(freq_data[i + 1]) - np.imag(freq_data[i])) ** 2), 2) for i in range(0, len(freq_data) - 1)]
        delta_p = [np.round(data_aggr[i + 1] - data_aggr[i], 2) for i in range(0, len(data_aggr) - 1)]
        print(delta_p)
        from collections import Counter
        c = Counter(delta_p)
        print(c)
        plt.hist(delta_p, bins=100)
        plt.yscale('log')
        plt.show()
